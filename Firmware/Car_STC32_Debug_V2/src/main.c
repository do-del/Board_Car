...
void sys_init()
{
	...
	//RF24初始化并进入RX模式
	RF24_Init();
	RF24_Write_Reg(WRITE_REG + EN_AA,0x01);	//管道0自动确认
	RF24_Write_Reg(WRITE_REG+EN_RXADDR,0x01);	//使能管道0
	RF24_Write_Reg(WRITE_REG+SETUP_RETR,0x0a);	//关闭自动重发
	RF24_RX_Mode();
	RF24_Set_Channel(66);	//选择射频通道66
	RF24_Set_Power(RF_PWR_0);	//选择发射功率
	RF24_Set_P0_Size(11);	//选择数据长度
	RF24_Set_Rate(RATE_1M_BPS);	//设置波特率
...
void common_isr() interrupt 13
{
	if(P2INTF & 0x04)
	{
		unsigned char rf24_status = 0;
		P2INTF = 0x00;
		
		CE_LOW();
		rf24_status = RF24_Read_Reg(READ_REG+STATUS);
		
		//接收中断
		if(rf24_status & RX_DR)
		{
			RF24_Read_Buf(RD_RX_PLOAD,rx,11);     //读取数据
			RF24_Write_Reg(WRITE_REG+STATUS, RX_DR);
			RF24_Write_Reg(FLUSH_RX, 0xff);
			printf("rx:%c\r\n", rx[0]);
			RF24_TX_Mode();
			CE_LOW();
			RF24_Write_Buf(WR_TX_PLOAD, tx, 11);
		...
		if(rf24_status & TX_DS)
		{
			RF24_Write_Reg(WRITE_REG+STATUS, TX_DS);
			RF24_Write_Reg(FLUSH_TX, 0xff);
			RF24_RX_Mode();
		}
		
		if(rf24_status & MAX_RT)
		{
			RF24_Write_Reg(WRITE_REG+STATUS, MAX_RT);
			RF24_Write_Reg(FLUSH_TX, 0xff);
			RF24_RX_Mode();
		}
	}
